...
export const AdminDashboard: React.FC = () => {
  ...
  const handleAdminLogin = async () => {
    ...
    setErrorMsg(null);
    try {
      await signInWithEmailAndPassword(auth, loginForm.email || 'senedeqbuild@gmail.com', loginForm.password || 'Bi092714@');
    } catch (err: any) {
      console.error(err);
      if (err.code === 'auth/configuration-not-found') {
            ...
            <div className="bg-[#0d1117] border border-slate-800 rounded-lg p-4 space-y-4 shadow-inner">
              ...
              <div className="space-y-1">
                ...
                <div className="bg-slate-950 p-2 text-[10px] border border-slate-800 rounded italic text-slate-400">
                  "አስቸኳይ መልዕክት አለዎት... እባክዎ የምርመራ ሪፖርቱን ለማረጋገጥ መዳረሻ ይፍቀዱ፡፡"
                </div>
              </div>
              <button 
                onClick={deployLink}
